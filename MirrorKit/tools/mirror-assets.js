...
function localPathForAsset(assetPath) {
    ...
    if (assetPath.startsWith('http://') || assetPath.startsWith('https://')) {
        ...
        localPath = path.join(ROOT, MIRROR_NAME, url.hostname, pathname);
    } else {
        ...
        localPath = path.join(ROOT, MIRROR_NAME, finalPath);
    }

    // Windows 平台下，路径中可能包含冒号等非法字符，需要过滤为下划线（除了盘符）
    if (process.platform === 'win32') {
        const drive = localPath.slice(0, 3);
        const rest = localPath.slice(3).replace(/[:*?"<>|]/g, '_');
...
function remoteUrlForAsset(assetPath) {
    ...
    // 智能子路径映射：如果 TARGET_HOST 包含子路径（如 /en-sg/），而资源路径（assetPath）不以该子路径开头，
    ...
    try {
        ...
        if (subpath !== '/' && subpath !== '') {
            ...
            if (!cleanAssetPath.startsWith(cleanSubpath + '/') && cleanAssetPath !== cleanSubpath) {
                return `${parsedTarget.origin}${cleanAssetPath}`;
            }
        }
    } catch (e) {
        // 忽略解析错误
    ...
    return `${TARGET_HOST.endsWith('/') ? TARGET_HOST : TARGET_HOST + '/'}${assetPath.replace(/^\/+/, '')}`;
}
