...
MirrorKit Hub 是一个强大的网页离线镜像与代理缓存框架。它能够将任意复杂的现代网站完整抓取到本地，并通过本地代理服务器实现像素级的离线浏览。

得益于增强的二进制资产保护机制与 SPA 路由适配能力，本项目不仅支持传统的静态 HTML 站点，更**支持以下复杂资源的抓取与本地渲染**：

- **3D 引擎与交互资源**：WebGL, Three.js, 模型文件 (`.gltf`, `.glb`, `.obj`, `.bin`)、高精度纹理贴图。
- **现代前端框架 (SPA)**：React Router, Astro, Next.js, Vue 的动态按需加载分块 (`chunk.js`)。
- **二进制流与多媒体**：无扩展名或强效混淆的商业字体 (如 Adobe Typekit / `wOFF2` 魔数探针)、全屏背景视频 (`.mp4`, `.webm`)。
- **前端动态接口响应**：CMS 隐藏媒体数据、页面初始化所需的动态 Fetch/XHR JSON 数据接口拦截与持久化缓存。

**核心引擎规则：**

```text
所有请求先走本地缓存 → 本地没有，再去远程代理拉取 → 请求成功后，将媒体/代码缓存到本地物理目录 → 以后再访问，直接从本地极速读取
```
...
- **重写隔离**：仅对 HTML 类文本进行 URL 重写，二进制资源（字体、3D 模型、纹理）100% 绕过文本替换引擎，防止数据损坏
